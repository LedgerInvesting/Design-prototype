...
interface TransactionBDXAggregatedMappingProps {
...
export const TransactionBDXAggregatedMapping: React.FC<TransactionBDXAggregatedMappingProps> = ({
  onNavigateToPage,
  transactionData
}) => {
  ...
  React.useEffect(() => {
    // Always open the modal when this page loads (coming from Add button)
    setIsImportModalOpen(true);
  }, []); // Empty dependency array so it only runs once on mount

  // Create navigation handler
  const navigationHandler = onNavigateToPage
    ? createPageNavigationHandler(onNavigateToPage, 'transaction-bdx-aggregated-mapping')
    : undefined;
  ...
  const stepper: StepConfig[] = [
    { label: 'Add data', status: 'active' },
    { label: 'Map data', status: 'disabled' },
    { label: 'Validate & submit', status: 'disabled' }
  ];

  // Create breadcrumbs with back navigation to transaction dashboard
  const breadcrumbs = onNavigateToPage
    ? [
        { label: transactionName, isActive: false },
        { label: 'BDX Upload', onClick: () => onNavigateToPage('transaction-dashboard'), isActive: false },
        { label: 'Aggregated Mapping', isActive: true }
      ]
    : [
  ...
  return (
    <Layout
      ...
      pageType="transaction-bdx-aggregated-mapping"
          ...
          <div style={{
            ...
            <Button
              ...
              onClick={() => {
                ...
                if (onNavigateToPage) {
                  // Pass the metrics values to the cession statement page with aggregated flow type
