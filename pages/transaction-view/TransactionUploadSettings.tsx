...
export interface TransactionUploadSettingsProps {
  onNavigateToPage?: (page: string, data?: any) => void;
  transactionName?: string;
  initialTab?: string;
}

export const TransactionUploadSettings: React.FC<TransactionUploadSettingsProps> = ({
  onNavigateToPage,
  transactionName,
  initialTab
}) => {
  ...
  const [activeTab, setActiveTab] = useState<string>(initialTab || 'basic-info');
  ...
  const [dataDetailLevel, setDataDetailLevel] = useState<string>('detail');
  ...
  // Get transaction-specific localStorage key
  const getSettingsKey = () => {
    if (!transactionName) return 'transaction-settings-default';
    return `transaction-settings-${transactionName.toLowerCase().replace(/\s+/g, '-')}`;
  };
  ...
  useEffect(() => {
    ...
    if (savedSettings) {
      ...
      setDataDetailLevel(settings.dataDetailLevel || 'detail');
      setFrequencyValue(settings.frequency || 'monthly');
      ...
      setDataDetailLevel(defaultDetailLevel);
      console.log(`No saved settings for ${transactionName}, using default: ${defaultDetailLevel}`);
    }
  }, [transactionName]);
  ...
  useEffect(() => {
    if (premiumTagging || lossesTagging || lossesExpenses || lossesRecoveries || dataDetailLevel || frequencyValue) {
      const settings = {
        premiumTagging,
        lossesTagging,
        lossesExpenses,
        lossesRecoveries,
        dataDetailLevel: dataDetailLevel || 'detail',
        frequency: frequencyValue || 'monthly'
      };
      const settingsKey = getSettingsKey();
      localStorage.setItem(settingsKey, JSON.stringify(settings));
      console.log(`Saved settings for ${transactionName}:`, settings);
    }
  }, [premiumTagging, lossesTagging, lossesExpenses, lossesRecoveries, dataDetailLevel, frequencyValue, transactionName]);
  ...
  return (
    <FormLayout
      formTitle={transactionName || 'Upload Settings'}
      ...
      backButtonText="Back to BDX Upload"
      onBackClick={() => {
        console.log('Back to BDX Upload clicked');
        onNavigateToPage?.('transaction-dashboard');
      }}
...
export default TransactionUploadSettings;